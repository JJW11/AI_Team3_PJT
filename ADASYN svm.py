...
target_defect_ratio = 0.5 # -> Minority Class 의 비율을 0.5 까지 끌어 올리겠다

# Stable ADASYN 구현 : 샘플링 기준을 결정하여 랜덤적 요소를 줄인 안정적인 oversampling 수행
class stable_ADASYN:
    ...
    def fit_sample(self, x_dataset, y_dataset):
            ...
            for index, row in defect_dataset.iterrows():
                ...
                sort_dataset = copy_dataset.sort_values(by="distance", ascending=True)
                nearest_dataset = sort_dataset.iloc[1: self.z_nearest + 1]
                # normalized ratio 계산 및 생성할 샘플 수 결정
                majority_number = len(nearest_dataset[nearest_dataset["bug"] == 0])
                ratio = majority_number / self.z_nearest
...
# Out-of-sample bootstrap 방식으로 훈련/테스트 데이터 분리
def separate_data(original_data):
    ...
    # 선택되지 않은 데이터를 테스트 데이터로 사용 (약 36.8%)
...
auc_file = open('output_data/ADASYN_svm/'+str(neighbor)+'auc_adasyn_result_on_'+classifier+'.csv', 'w', newline='')
auc_writer = csv.writer(auc_file)
auc_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

balance_file = open('output_data/ADASYN_svm/'+str(neighbor)+'balance_adasyn_result_on_'+classifier+'.csv', 'w', newline='')
balance_writer = csv.writer(balance_file)
balance_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

recall_file = open('output_data/ADASYN_svm/'+str(neighbor)+'recall_adasyn_result_on_'+classifier+'.csv', 'w', newline='')
recall_writer = csv.writer(recall_file)
recall_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

pf_file = open('output_data/ADASYN_svm/'+str(neighbor)+'pf_adasyn_result_on_'+classifier+'.csv', 'w', newline='')
pf_writer = csv.writer(pf_file)
pf_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max"])

brier_file = open('output_data/ADASYN_svm/'+str(neighbor)+'brier_adasyn_result_on_'+classifier+'.csv', 'w', newline='')
brier_writer = csv.writer(brier_file)
brier_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

mcc_file = open('output_data/ADASYN_svm/'+str(neighbor)+'mcc_adasyn_result_on_'+classifier+'.csv', 'w', newline='')
mcc_writer = csv.writer(mcc_file)
mcc_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

stable_auc_file = open('output_data/ADASYN_svm/'+str(neighbor)+'auc_stable_adasyn_result_on_'+classifier+'.csv', 'w',
              newline='')
stable_auc_writer = csv.writer(stable_auc_file)
stable_auc_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

stable_balance_file = open('output_data/ADASYN_svm/'+str(neighbor)+'balance_stable_adasyn_result_on_'+classifier+'.csv', 'w',
              newline='')
...
stable_recall_file = open('output_data/ADASYN_svm/'+str(neighbor)+'recall_stable_adasyn_result_on_'+classifier+'.csv', 'w',
              newline='')
...
stable_pf_file = open('output_data/ADASYN_svm/'+str(neighbor)+'pf_stable_adasyn_result_on_'+classifier+'.csv', 'w',
              newline='')
stable_pf_writer = csv.writer(stable_pf_file)
stable_pf_writer.writerow(["inputfile", "", "", "", "", "", "", "", "", "", "", "min", "lower", "avg", "median", "upper", "max", "variance"])

stable_brier_file = open('output_data/ADASYN_svm/'+str(neighbor)+'brier_stable_adasyn_result_on_'+classifier+'.csv', 'w',
              newline='')
...
stable_mcc_file = open('output_data/ADASYN_svm/'+str(neighbor)+'mcc_stable_adasyn_result_on_'+classifier+'.csv', 'w',
              newline='')
...
for inputfile in os.listdir("input_data/"):
    ...
    y = dataset["bug"]
    # 하이퍼파라미터 튜닝을 위한 validation 데이터 분리
    ...
    for j in range(10):
        ...
        # 각 train/test split에 대해 10번 반복 실험
        for s in range(10):
        ...
        # 통계값 계산 및 CSV 파일에 기록 - Brier Score
        ...
        # PF (False Positive Rate) 통계값 계산 및 기록
        max_pf = max(pf_row)
        min_pf = min(pf_row)
        ...
        # MCC (Matthews Correlation Coefficient) 통계값 계산 및 기록
        max_mcc = max(mcc_row)
        min_mcc = min(mcc_row)
